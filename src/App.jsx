				...
				axios.all(promises)
					.then(
						axios.spread((...allData) => {
							...
							currCards = currCards.filter(e => {
								let search_split = searchParams.get('search').split(" ")
								search_split = search_split.map(v => v.toLowerCase().replace('-',"").replace('/',''))
								for (const value of Object.keys(e)) {
									if ((typeof e[value] == "string" || typeof e[value] == "number") && e[value]) {
										let value_split = String(e[value]).split(" ")
										value_split = value_split.map(v => v.toLowerCase().replace('-',"").replace('/',""))
										for (const split of value_split) {
											if (search_split.includes(split)) {
												return true
											}
										}
									} else if (value == "variações") {
										for (let i = e['variações']; i < e['variações'].length; i++) {
											for (const valueVar of Object.keys(e['variações'][i])) {
												if ((typeof e['variações'][valueVar] == "string" || typeof e['variações'][valueVar] == "number") && e['variações'][valueVar]) {
													const value_split = String(e['variações'][valueVar]).split(" ").replace('-','').replace('/','')
													for (const split of value_split) {
														if (search_split.includes(split)) {
															return true
														}
